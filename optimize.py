...
def optimize_image(input_file, output_path=None, jpg_quality=OPTIMIZE_JPG_QUALITY, webp_quality=OPTIMIZE_WEBP_QUALITY, optimize=True, overwrite=False, lossless=OPTIMIZE_LOSSLESS):
...
def get_optimize_parser():
    ...
    parser.add_argument("--jpg-quality", type=int, default=OPTIMIZE_JPG_QUALITY, help=f"JPEG image quality (1-100, default: {OPTIMIZE_JPG_QUALITY})")
    parser.add_argument("--webp-quality", type=int, default=OPTIMIZE_WEBP_QUALITY, help=f"WebP image quality (1-100, default: {OPTIMIZE_WEBP_QUALITY}, ignored when --lossless option is used)")
    parser.add_argument("--lossless", action="store_true", default=OPTIMIZE_LOSSLESS, help="Use lossless compression for WebP and PNG (ignores WebP quality setting)")
    return parser

def main():
    parser = get_optimize_parser()
    ...
    try:
        if os.path.isfile(input_file_abs):
            ...
            optimize_image(input_file_abs, output_path_abs, jpg_quality=args.jpg_quality, webp_quality=args.webp_quality, optimize=True, overwrite=args.overwrite, lossless=args.lossless)
