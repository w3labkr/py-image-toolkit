...
class Config:
    ...
    output_format: str # Target output format (e.g., 'jpg', 'png'). Default is 'original'.
    input_dir: str = 'input' # Default input directory.
    output_dir_arg: Optional[str] = None # User-specified output directory (if any).
    resize_mode: str = 'aspect_ratio' # Default resize mode.
    ...
    def _prepare_options(self):
        ...
        if self.resize_mode != 'none':
            # self.filter will have a default value ('lanczos') if not specified by user
            self.resize_options.update({
                'width': self.width,
                'height': self.height,
                'filter_str': self.filter, 
                'filter_obj': _PIL_RESAMPLE_FILTERS[self.filter] # self.filter is guaranteed to be a valid key
            })
...
def get_unique_filepath(filepath: str) -> str:
    ...
    Example: "image.jpg" -> "image_1.jpg", "image_2.jpg", etc.

    Args:
        filepath: The desired filepath.

    Returns:
        A unique filepath, either the original or with an appended counter.
    """
    ...
    while True:
        ...
        if not os.path.exists(new_filepath):
            logger.debug(f"Filename conflict detected for '{filename}'. Renamed to '{new_filename}'.")
            return new_filepath
        counter += 1
...
def process_single_image_file(input_path: str, relative_path: str, config: Config) -> Tuple[bool, Optional[str], str, str]:
    ...
    if os.path.exists(output_path_base):
        if config.overwrite_policy == "skip":
            logger.debug(f"Skipping existing file due to 'skip' policy: '{relative_path}' (target: '{output_path_base}')")
            ...
            if output_path != output_path_base: 
                 logger.debug(f"Output file '{os.path.basename(output_path_base)}' exists. Renaming to '{os.path.basename(output_path)}' due to 'rename' policy.")
        elif config.overwrite_policy == "overwrite":
            logger.debug(f"Overwriting existing file due to 'overwrite' policy: '{output_path_base}'")
    ...
    try:
        with Image.open(input_path) as img:
            ...
            save_kwargs = {}
            save_format_arg = output_format_str.upper() if output_format_str != 'original' else None
        ...
        logger.critical(f"Processing failed for '{relative_path}': {msg}", exc_info=True) 
        return False, msg, input_path, relative_path
...
def scan_for_image_files(config: Config) -> Tuple[list[Tuple[str, str]], list[str]]:
    ...
    This version is always non-recursive.
    ...
    for filename in os.listdir(config.absolute_input_dir):
        ...
        if os.path.isfile(abs_path):
             items_found_to_evaluate.append((abs_path, filename)) 
...
def batch_process_images(config: Config) -> Tuple[int, int, int, list[Tuple[str, str]], list[str]]:
    ...
    It scans for files, creates necessary output subdirectories (though less relevant for non-recursive), 
    ...
    try:
        ...
        logger.critical(f"(!) Critical Error: Failed during file scanning or input path access for '{config.absolute_input_dir}'. Error: {e}", exc_info=True)
        return 0, 0, 0, [], all_skipped_scan_items_reasons
    ...
    logger.info(f"Batch processing complete. Success: {processed_count}, Errors: {error_count}, Skipped (due to overwrite policy): {skipped_overwrite_count}")
...
def parse_arguments() -> Config:
    ...
    mode_group = parser.add_argument_group('Resize Mode')
    mode_group.add_argument("-m", "--resize-mode", choices=['aspect_ratio', 'fixed', 'none'], default='aspect_ratio',
                                help="Resize mode (default: aspect_ratio):\n"
    ...
    optional_group.add_argument("--overwrite-policy", choices=['rename', 'overwrite', 'skip'], default='rename',
                                help="Action to take if an output file already exists:\n"
                                     "  rename: Save with a new name (e.g., image_1.jpg) (default).\n"
                                     "  overwrite: Replace the existing file.\n"
                                     "  skip: Do not process the image if output file exists.")
    ...
    if args.resize_mode == 'aspect_ratio':
        if args.width <= 0 and args.height <= 0:
            parser.error("For 'aspect_ratio' mode, either --width or --height (or both) must be greater than 0.")
        if args.width < 0 or args.height < 0: 
            parser.error("--width and --height cannot be negative values.")
    elif args.resize_mode == 'fixed':
        if args.width <= 0 or args.height <= 0:
            parser.error("For 'fixed' mode, both --width and --height must be greater than 0.")
...
def display_settings(config: Config):
    ...
    if config.resize_mode != 'none':
        ...
        size_desc = ""
        if config.resize_mode == 'aspect_ratio':
            size_desc = ' and/or '.join(size_info_parts) + " (maintaining aspect ratio)"
        elif config.resize_mode == 'fixed':
...
def print_summary(processed: int, errors: int, skipped_overwrite: int, error_list: list, skipped_scan_list_reasons: list, output_dir: str):
    ...
    if skipped_scan_list_reasons:
        ...
        display_skipped_reasons = skipped_scan_list_reasons[:5] 
...
def main():
    ...
    try:
        ...
        logger.info(f"===== Image Resizer Script v{__version__} Started =====")
        ...
        logger.info(f"===== Image Resizer Script v{__version__} Finished =====")
        ...
        print(f"\n(!) Critical Error: {e}. Check logs for details.") 
        sys.exit(2) 
