...
# --- Constants ---
__version__ = "3.38" # Script version (Add image dimensions to output filename)
...
class Config:
    ...
    overwrite: bool = False # Policy for handling existing output files. True to overwrite, False to skip.
    input_dir: str = 'input' # Default input directory.
    output_dir: Optional[str] = None # User-specified output directory (if any).
...
def process_single_image_file(input_path: str, relative_path: str, config: Config) -> Tuple[bool, Optional[str], str, str]:
    ...
    converts format (if applicable), and saves it to the output directory with dimensions in the filename.
    ...
    # Determine the final output extension for the filename
    ...
    if output_format_str.lower() == 'original':
        ...
        if final_output_ext is None: # Fallback, though argparse should prevent this
    ...
    # Initialize output_path. It will be updated after image dimensions are known.
    # This initial value is a placeholder in case of very early errors before dimensions are read.
    ...
    try:
        with Image.open(input_path) as img:
            ...
            # Get final dimensions from the (potentially) resized image
            final_width, final_height = img_resized.size
            # Construct the new output filename with dimensions: 파일명_가로x세로.확장자
            output_filename_with_dims = f"{base_name}_{final_width}x{final_height}{final_output_ext}"
            # Update output_path to the new name
            output_path = os.path.join(output_dir_for_file, output_filename_with_dims)
            # --- END FILENAME MODIFICATION ---

            if os.path.exists(output_path):
                if config.overwrite:
                    logger.debug(f"Output file '{output_filename_with_dims}' exists. Overwriting due to --overwrite policy.")
                else:
            ...
            if not config.strip_exif:
                # ... (EXIF processing logic for saving, same as before) ...
        ...
        logger.critical(f"Processing failed for '{relative_path}': {msg}", exc_info=config.verbose)
        return False, msg, input_path, relative_path
...
def parse_arguments() -> Config:
    ...
    """
    temp_config_for_defaults = Config(output_format='original', overwrite=False, filter='lanczos', ratio='aspect_ratio', _skip_post_init_actions=True)
...
def display_settings(config: Config):
    ...
    overwrite_action = "Overwrite existing files" if config.overwrite else "Skip existing files (default)"
    print(f"File Overwrite Policy (--overwrite): {overwrite_action}")
