...
import subprocess
import sys
from optimize import OPTIMIZE_JPG_QUALITY, OPTIMIZE_WEBP_QUALITY, OPTIMIZE_LOSSLESS, get_optimize_parser

SUPPORTED_EXTENSIONS = ('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff', '.webp')

def run_batch_optimization(input_dir, output_dir="output", jpg_quality=OPTIMIZE_JPG_QUALITY, webp_quality=OPTIMIZE_WEBP_QUALITY, lossless=OPTIMIZE_LOSSLESS, overwrite=False):
    ...
    optimize_script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "optimize.py")
    if not os.path.isfile(optimize_script_path):
        print(f"'optimize.py' script not found. It should be in the same directory as 'batch_optimize.py'.")
        sys.exit(1)

    print(f"Input directory: {input_dir}")
    print(f"Output directory: {output_dir}")
    print(f"JPEG quality: {jpg_quality}, WebP quality: {webp_quality}, Lossless: {lossless}, Overwrite: {overwrite}")
    print("-" * 30)
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_dir", help="Input directory path containing images to optimize")
