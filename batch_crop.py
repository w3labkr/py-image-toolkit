...
import subprocess
import sys
from crop import get_crop_parser, CROP_SUPPORTED_EXTENSIONS

def run_batch_crop(input_dir, output_dir="output", overwrite=False,
                     rule=None, ratio=None, padding_percent=None,
                     reference=None, method=None,
                     min_face_width=None, min_face_height=None, model_path=None):
    ...
    crop_script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "crop.py")
    if not os.path.isfile(crop_script_path):
        print(f"'crop.py' script not found. It should be in the same directory as 'batch_crop.py'.")
        sys.exit(1)
    ...
    print(f"Overwrite: {overwrite}, Rule: {rule}, Ratio: {ratio}, Padding: {padding_percent}%, Reference: {reference}, Subject selection method: {method}")
    print(f"Min face width: {min_face_width}, Min face height: {min_face_height}, Model path: {display_model_path}")
    print("-" * 30)
...
if __name__ == "__main__":
    ...
    parser.add_argument("input_dir", help="Input directory path containing images to crop")
