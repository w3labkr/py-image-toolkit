...
def process_image_file(input_path: str, output_path: str, resize_opts: dict) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        return False, "Invalid or corrupt image file. Pillow cannot recognize the image format."
...
def execute_resize_operation(input_file: str, output_file: str, overwrite: bool, resize_mode: str,
                       width: int, height: int, filter_str: str, filter_obj: Any) -> NoReturn:
    try:
        if not overwrite and os.path.exists(output_file):
            print(f"'{os.path.basename(output_file)}' file already exists and overwrite option is disabled, skipping.")
            sys.exit(0)
...
def get_resize_parser():
    ...
    parser.add_argument("-o", "--output-dir", help="Path to save resized images", default="output")
