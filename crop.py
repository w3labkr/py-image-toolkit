...
YUNET_MODEL_URL: str = (
    f"https://github.com/opencv/opencv_zoo/raw/main/models/face_detection_yunet/{YUNET_MODEL_FILENAME}"
)
OUTPUT_SUFFIX_THIRDS: str = "_thirds"
OUTPUT_SUFFIX_GOLDEN: str = "_golden"
MODEL_DIR_NAME: str = "models"
CROP_SUPPORTED_EXTENSIONS: Tuple[str, ...] = (
    ".png",
    ".jpg",
    ".jpeg",
    ".bmp",
...
def download_model(model_url: str, file_path: str, is_main_process: bool) -> bool:
    ...
    if not os.path.exists(model_dir):
        try:
            ...
            print(
                f"Failed to create model directory '{os.path.relpath(model_dir)}': {e}"
            )
            return False
    ...
    if not os.path.exists(file_path):
        if is_main_process:
            print(
                f"Downloading model file... ({os.path.basename(file_path)}) from {model_url}"
            )
        try:
            ...
            if is_main_process:
                print(
                    f"Model file downloaded successfully: {os.path.relpath(file_path)}"
                )
            return True
        except Exception as e:
            print(f"Model file download failed: {e}")
            print(
                f"Please manually download from the following URL and save as '{os.path.relpath(file_path)}': {model_url}"
            )
            return False
    return True
...
def parse_aspect_ratio(ratio_str: Optional[str]) -> Optional[float]:
    ...
    try:
        ...
        if ":" in ratio_str:
            ...
            if h <= 0 or w <= 0:
                print(
                    f"Invalid ratio values in '{ratio_str}'. Width and height must be positive. Using original ratio."
                )
                return None
            return w / h
        else:
            ...
            if ratio <= 0:
                print(
                    f"Invalid ratio value '{ratio_str}'. Ratio must be positive. Using original ratio."
                )
                return None
            return ratio
    except ValueError:
...
def detect_faces_dnn(
    detector: cv2.FaceDetectorYN, image: np.ndarray, min_w: int, min_h: int
) -> List[Dict[str, Any]]:
    detected_subjects = []
...
def select_main_subject(
    ...
    try:
        if len(subjects) == 1:
            ...
            best_subject = min(
                subjects,
                key=lambda s: math.dist(s.get("bbox_center", (0, 0)), img_center),
            )
        else:
        ...
        if best_subject:
            ...
            is_eye_center_valid = (
                isinstance(eye_center_val, tuple) and len(eye_center_val) == 2
            )
            is_bbox_center_valid = (
                isinstance(bbox_center_val, tuple) and len(bbox_center_val) == 2
            )
...
def get_rule_points(
    width: int, height: int, rule_type: str = "thirds"
) -> List[Tuple[int, int]]:
    points = []
    if width <= 0 or height <= 0:
        print(f"Cannot calculate rule points for invalid size ({width}x{height}).")
        return []
    try:
        if rule_type == "thirds":
            points = [
        ...
        print(
            f"Error calculating rule points (Rule: {rule_type}, Size: {width}x{height}): {e}"
        )
        return []
...
def calculate_optimal_crop(
    ...
    rule_points: List[Tuple[int, int]],
    target_aspect_ratio: Optional[float],
) -> Optional[Tuple[int, int, int, int]]:
    height, width = img_shape
    if height <= 0 or width <= 0:
        print(f"Cannot crop image with zero/negative dimensions ({width}x{height}).")
        return None
    if not rule_points:
        print("No rule points provided, skipping crop calculation.")
        return None
    ...
    try:
        ...
        if aspect_ratio <= 0:
            print(
                f"Invalid target aspect ratio ({aspect_ratio}). Cannot calculate crop."
            )
            return None
        ...
        if max_w_for_target <= 0 or max_h_for_target <= 0:
            print(
                f"Cannot create a valid crop centered at rule point ({target_x},{target_y}) as it's too close to image edge or image dimensions are too small."
            )
            return None
        ...
        x1_raw = target_x - final_w / 2.0
        y1_raw = target_y - final_h / 2.0
        x2_raw = x1_raw + final_w
        y2_raw = y1_raw + final_h
        ...
        if x1 >= x2 or y1 >= y2:
            print(
                f"Calculated crop area has zero/negative size ({x1},{y1}) to ({x2},{y2}). Raw was ({x1_raw:.2f},{y1_raw:.2f}) to ({x2_raw:.2f},{y2_raw:.2f})."
            )
            return None
        return x1, y1, x2, y2
    except Exception as e:
...
def apply_padding(
    ...
    if new_x1 >= new_x2 or new_y1 >= new_y2:
        print(
            f"Crop area invalid after {padding_percent}% padding. Original: ({x1},{y1})-({x2},{y2}). Padded: ({new_x1},{new_y1})-({new_x2},{new_y2}). No padding applied."
        )
        return crop_coords
...
def _load_and_prepare_image(
    ...
    try:
        ...
        print(f"{filename}: Cannot open or unsupported image format.")
        return None, None
    except Exception as e:
...
def _save_cropped_image(
    cropped_bgr: np.ndarray, out_path: str, settings: argparse.Namespace
) -> bool:
    ...
    try:
        ...
        print(f"{filename}: File write error: {e}")
        return False
    except ValueError as e:
        print(f"{filename}: Cropped image processing error: {e}")
        return False
    except Exception as e:
        print(f"{filename}: Error saving cropped image: {e}")
        return False


def _process_composition_rule(
    rule_name: str,
    rule_suffix: str,
    img_bgr: np.ndarray,
    ref_center: Tuple[int, int],
    current_settings: argparse.Namespace,
    original_ext: str,
) -> Tuple[bool, bool, str]:
    filename = os.path.basename(current_settings.input_file)
    base_filename = os.path.splitext(filename)[0]
    img_h, img_w = img_bgr.shape[:2]
    saved_actual_file = False
    was_skipped_overwrite = False
    error_message = ""
    ...
    crop_coords = calculate_optimal_crop(
        (img_h, img_w), ref_center, rule_points, current_settings.target_ratio_float
    )
    if not crop_coords:
        error_message = f"Rule '{rule_name}': Failed to calculate optimal crop."
        print(f"{filename}: {error_message}")
        return saved_actual_file, was_skipped_overwrite, error_message

    padded_coords = apply_padding(
        crop_coords, (img_h, img_w), current_settings.padding_percent
    )
    x1, y1, x2, y2 = padded_coords
    if x1 >= x2 or y1 >= y2:
        error_message = f"Rule '{rule_name}': Final crop area invalid after padding."
        print(f"{filename}: {error_message}")
        return saved_actual_file, was_skipped_overwrite, error_message

    out_filename, output_ext = _determine_output_filename(
        base_filename, rule_suffix, current_settings, original_ext
    )
    ...
    if not current_settings.overwrite and os.path.exists(out_path):
        error_message = (
            f"Rule '{rule_name}': Skipped (file exists, overwrite disabled)."
        )
...
def process_image(
    image_path: str, global_settings: argparse.Namespace, detector: cv2.FaceDetectorYN
) -> Dict[str, Any]:
    filename = os.path.basename(image_path)
    ...
    detected_faces = detect_faces_dnn(
        detector,
        img_bgr,
        current_settings.min_face_width,
        current_settings.min_face_height,
    )
    if not detected_faces:
        status["message"] = "No valid faces detected (considering minimum size)."
        return status

    selection_result = select_main_subject(
        detected_faces,
        (img_h, img_w),
        current_settings.method,
        current_settings.reference,
    )
    ...
    rules_to_process = []
    if current_settings.rule in ["thirds", "both"]:
        rules_to_process.append(("thirds", OUTPUT_SUFFIX_THIRDS))
    if current_settings.rule in ["golden", "both"]:
        rules_to_process.append(("golden", OUTPUT_SUFFIX_GOLDEN))
    if not rules_to_process:
        status["message"] = f"No composition rule selected ('{current_settings.rule}')."
        return status

    saved_rules_count = 0
    skipped_overwrite_count_for_image = 0
    rule_errors = []
    for rule_name, rule_suffix in rules_to_process:
        saved, skipped_overwrite, error_msg = _process_composition_rule(
            rule_name, rule_suffix, img_bgr, ref_center, current_settings, original_ext
        )
        if saved:
            saved_rules_count += 1
        if skipped_overwrite:
            skipped_overwrite_count_for_image += 1
        if error_msg and not saved and not skipped_overwrite:
    ...
    if saved_rules_count > 0:
        ...
        if skipped_overwrite_count_for_image > 0:
            status[
                "message"
            ] += f", skipped {skipped_overwrite_count_for_image} rule(s) (overwrite)"
        if rule_errors:
            status["message"] += f". Errors: {'; '.join(rule_errors)}"
    elif skipped_overwrite_count_for_image > 0:
        status["success"] = False
        status["message"] = (
            f"Skipped {skipped_overwrite_count_for_image} rule(s) (overwrite policy)"
        )
        if rule_errors:
        ...
        err_summary = (
            "; ".join(rule_errors)
            if rule_errors
            else "Crop/Save failed or no faces/rules applicable."
        )
...
def run(settings: argparse.Namespace) -> Dict[str, int]:
    ...
    if os.path.isfile(input_file):
        if not input_file.lower().endswith(CROP_SUPPORTED_EXTENSIONS):
            print(
                f"File '{os.path.relpath(input_file)}' is not a supported image type. Skipping."
            )
            return summary
...
def get_parser():
    ...
    parser.add_argument(
        ...
        default="largest",
        help="Method to select main subject. Default: largest.",
    )
    parser.add_argument(
        "--ref",
        "--reference",
        ...
        choices=["eye", "box"],
        default="box",
        help="Reference point for composition. Default: box.",
    )
    parser.add_argument(
        "-c",
        "--confidence",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Face detection NMS threshold. Default: 0.3.",
    )
    parser.add_argument(
        "--min-face-width",
        type=int,
        default=30,
        help="Min face width in pixels. Default: 30.",
    )
    parser.add_argument(
        "--min-face-height",
        type=int,
        default=30,
        help="Min face height in pixels. Default: 30.",
    )
    parser.add_argument(
        "--rule",
        choices=["thirds", "golden", "both"],
        default="both",
        help="Composition rule(s) to apply. Default: 'both'.",
    )
    parser.add_argument(
        "--ratio",
        type=str,
        default=None,
        help="Target aspect ratio (e.g., '16:9', '4:3', '1:1', or 'None' for original). Default: None (original).",
    )
    parser.add_argument(
        "--padding-percent",
        type=float,
        default=0.0,
        help="Padding percentage around the final crop (0.0 to 100.0). Default: 0.0.",
    )
    parser.add_argument(
        "--yunet-model-path",
        type=str,
